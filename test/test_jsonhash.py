...
DATA = [
    ...
    {'a': 1, 'b': 2, 'c': 3, 'd': 4},
    [1,3,2,5,43,3],
    {'a': 'just some dict', '1': ['with lists', 1, 'and intergeres', {'a': {'and even more': 'stuff', 'c': 123}}]}
]
...
def compare(data0, data1, algorithm=None):
    assert jsonhash.hash(data0, algorithm).hexdigest() == jsonhash.hash(data1, algorithm).hexdigest()
